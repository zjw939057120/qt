...
void Modbus_MasterRecvProcess(unsigned char chan,unsigned char addr)
{
	...
	while(nlen >= 0x08)
	{
		if((VariaDlg.RecvBuff[chan][VariaDlg.ReadPtr[chan]] == addr) && (VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+1)&RECV_BUFF_LEN] == 0x06))		//写单个寄存器
		{
			StartAddr = (unsigned short)VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+2)&RECV_BUFF_LEN] * 256 + VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+3)&RECV_BUFF_LEN] ;	//开始地址
		...
		else if((VariaDlg.RecvBuff[chan][VariaDlg.ReadPtr[chan]] == addr) && (VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+1)&RECV_BUFF_LEN] == 0x10))		//写多个寄存器
		{
			StartAddr = (unsigned short)VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+2)&RECV_BUFF_LEN] * 256 + VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+3)&RECV_BUFF_LEN] ;	//开始地址
			RegNumber = (unsigned short)VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+4)&RECV_BUFF_LEN] * 256 + VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+5)&RECV_BUFF_LEN] ;	//寄存器个数																						
			EndAddr = StartAddr + RegNumber - 1 ;																//寄存器的结束地址
		...
		else if((VariaDlg.RecvBuff[chan][VariaDlg.ReadPtr[chan]] == addr) && (VariaDlg.RecvBuff[chan][(VariaDlg.ReadPtr[chan]+1)&RECV_BUFF_LEN] == 0x03))		//读多个寄存器
		{
			...
			crc2 =  VariaDlg.HandBuff[chan][PacketSize-2] + (unsigned short)VariaDlg.HandBuff[chan][PacketSize-1]*256 ;		
			if(crc1 == crc2)								//计算CRC
			{
				VariaDlg.ReadPtr[chan] = HandPtr;	
				...
				if (nSize == VariaDlg.RegNumber)
				{
					...
					{
						VariaDlg.Register[StartAddr + i] = GetWORDBuffer((unsigned char *)&VariaDlg.HandBuff[chan][3 + 2 * i], 1);
					}
				}
...
//	生成modbus协议数据流
//================================================================================================================
unsigned short Modbus_MasterMakeProcess(unsigned char chan,unsigned char addr,char CMDType,unsigned short StartAddr,unsigned short RegNumber)
{
...
void Modbus_MasterSendProcess(void)
{
	...
	if (TaskDlg.WritePtr != TaskDlg.ReadPtr)				//有指令需要发送
	{
		if (!TaskDlg.ParaBuff[TaskDlg.ReadPtr].Busy)
		{
			...
			VariaDlg.SendBuffLen[TaskDlg.ParaBuff[TaskDlg.ReadPtr].Chan] = Modbus_MasterMakeProcess(TaskDlg.ParaBuff[TaskDlg.ReadPtr].Chan,TaskDlg.ParaBuff[TaskDlg.ReadPtr].Addr, TaskDlg.ParaBuff[TaskDlg.ReadPtr].Code, TaskDlg.ParaBuff[TaskDlg.ReadPtr].StartAddr, TaskDlg.ParaBuff[TaskDlg.ReadPtr].RegNumber);
			Modbus_SendBuff(TaskDlg.ParaBuff[TaskDlg.ReadPtr].Chan, VariaDlg.SendBuff[TaskDlg.ParaBuff[TaskDlg.ReadPtr].Chan], VariaDlg.SendBuffLen[TaskDlg.ParaBuff[TaskDlg.ReadPtr].Chan], TaskDlg.ParaBuff[TaskDlg.ReadPtr].Addr);
		}
		else
		{
	...
	else
	{
		if (!VariaDlg.OrderBusy)
		{
			...
			if (VariaDlg.SampleDataReadTime >= nTime)			//读数据
			{
				...
				VariaDlg.SendBuffLen[CHAN_COMM] = Modbus_MasterMakeProcess(CHAN_COMM,VariaDlg.OrderAddr, VariaDlg.OrderCode, VariaDlg.StartAddr, VariaDlg.RegNumber);
				Modbus_SendBuff(CHAN_COMM, VariaDlg.SendBuff[CHAN_COMM], VariaDlg.SendBuffLen[CHAN_COMM], VariaDlg.OrderAddr);
			}
		}
		else
		{
